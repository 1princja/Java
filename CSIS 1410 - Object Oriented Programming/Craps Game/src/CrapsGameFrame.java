/********************************************************
 *  Project :  Assignment 13- GUI-based Craps Program
 *  File    :  CrapsGameFrame.java
 *  Name    :  Anthony Browness
 *  Date    :  17 April 2013
 *
 *  Description 	: A Craps game presented in a GUI.  Contains a menu bar with:
 *  					File: New user, Exit
 *  					Game Stats: Ttoal games, Total wins, Avg Rolls/Game, Summary
 *  					Preferences: Background color, Look & Feel
 *  					About: Instructions, Project  
 *
 *	Data Structures	: Arrays for Background color and Look & Feel
 *
 *  Implementation	:  The Program requires the user to select a new user from the file button, it will then prompt the user to input a user name which will
 *  					then be displayed on the interface.  The New Game button will become enabled and the player can start playing the game.  After the New Game button
 *  					is pressed the Comeout button is enabled and the New Game button is disabled.  The player does a Comeout roll which will set the point value to be checked
 *  					against in the future.  All player rolls show the individual dice values on the screen and the total of the dice which can be compared to the point value.
 *  					If the player dose'nt immediatly win or lose on the comeout roll then the Comeout roll button will be disabled and the Roll Again button will be enabled.
 *  					The player must press this button until he either matches his point or loses with a 7.   When a player either wins or loses a message is displayed on the screen
 *  					statistics are updated, and all buttons except newgame become disabled again.
 *  
 *  					Important:  The statistics summary and avg rolls/game will be disabled until at least one game is played, this ensures that the stats do not ever try to divide by 0.  
 *  
 *  Methods			: 	comeOut
 *  					rollAgain
 *  					winner
